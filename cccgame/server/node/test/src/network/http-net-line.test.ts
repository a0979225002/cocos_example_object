...
describe('NetServer', ()=>{
	...
	test('addServer/getServer/removeServer', async ()=>{
		...
		lcc.netServer.addServer('testServer', lcc.NetLineType.HTTP, {  port : await getEmptyPort() });
		lcc.netServer.addServer('testServer2', lcc.NetLineType.HTTP, {  port : await getEmptyPort() });
...
describe('NetClient', ()=>{
	...
	test('addClient/getClient/removeClient', async ()=>{
		...
		lcc.netClient.addClient('testClient', lcc.NetLineType.HTTP, {  url : 'http://localhost:4567' });
		lcc.netClient.addClient('testClient2', lcc.NetLineType.HTTP, {  url : 'http://localhost:4567' });
...
describe('net-line', ()=>{
	...
	test('writeProtocol/dealProtocol TYPESCRIPTTEST1 useProtoBuf', async ()=>{
		let tempPort = await getEmptyPort();
		let server = lcc.netServer.addServer('testServer', lcc.NetLineType.HTTP, {  port : tempPort });
		let client = lcc.netClient.addClient('testClient', lcc.NetLineType.HTTP, {  url : `http://localhost:${tempPort}` ,call : lcc.HttpCall.GET });
	...
	test('writeProtocol/dealProtocol TYPESCRIPTTEST1', async ()=>{
		let tempPort = await getEmptyPort();
		let server = lcc.netServer.addServer('testServer', lcc.NetLineType.HTTP, {  port : tempPort });
		let client = lcc.netClient.addClient('testClient', lcc.NetLineType.HTTP, {  url : `http://localhost:${tempPort}` ,call : lcc.HttpCall.POST });
