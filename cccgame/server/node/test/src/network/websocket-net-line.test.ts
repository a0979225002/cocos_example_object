...
let portTest2 = 6789;
async function getEmptyPort2() {
	while(true){
		if(await lcc.os.isEmptyPort(portTest2)){
			let r = portTest2;
			portTest2 ++;
			return r;
		}else{
...
describe('NetServer', ()=>{
	...
	test('addServer/getServer/removeServer', async ()=>{
		...
		lcc.netServer.addServer('testServer', lcc.NetLineType.WEBSOCKET, {  port : await getEmptyPort2() });
		lcc.netServer.addServer('testServer2', lcc.NetLineType.WEBSOCKET, {  port : await getEmptyPort2() });
...
describe('NetClient', ()=>{
	...
	test('addClient/getClient/removeClient', async ()=>{
		...
		lcc.netClient.addClient('testClient', lcc.NetLineType.WEBSOCKET, { url : "ws://localhost:4567" });
		lcc.netClient.addClient('testClient2', lcc.NetLineType.WEBSOCKET, { url : "ws://localhost:4567" });
...
describe('net-line', ()=>{
	...
	test('useProtoBuf/isUseProtoBuf', async ()=>{
		let clientLine = lcc.netClient.addClient('testClient', lcc.NetLineType.WEBSOCKET, { url : "ws://localhost:4567" });
	...
	test('writeProtocol/dealProtocol TYPESCRIPTTEST1 useProtoBuf', async ()=>{
		let tempPort = await getEmptyPort2();
		let server = lcc.netServer.addServer('testServer', lcc.NetLineType.WEBSOCKET, {  port : tempPort });
		let client = lcc.netClient.addClient('testClient', lcc.NetLineType.WEBSOCKET, { url : `ws://localhost:${tempPort}` });
