  ...
  return {
    ...
    execute: function () {
      ...
      _export("VerletPhysicsScene", VerletPhysicsScene = (_dec = (_crd && sampleScene === void 0 ? (_reportPossibleCrUseOfsampleScene({
        error: Error()
      }), sampleScene) : sampleScene)("Verlet Physics"), _dec(_class = class VerletPhysicsScene extends (_crd && RenderScene === void 0 ? (_reportPossibleCrUseOfRenderScene({
        error: Error()
      }), RenderScene) : RenderScene) {
        initialize() {
          const verletSystem = this.createEntity("verlet-system").addComponent(new (_crd && VerletSystem === void 0 ? (_reportPossibleCrUseOfVerletSystem({
            error: Error()
        ...
        createRope(world) {
          ...
          for (var i = 0; i < 10; i++) linePoints[i] = new es.Vector2(30 * i + 50, 10);

          const line = new es.LineSegments(linePoints, 0.3).pinParticleAtIndex(0);
